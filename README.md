...
두 샘플은 동일한 기능 집합을 목표로 한다. 현재 기준 기능별 제공 현황은 다음과 같다.

| 기능 | spring-ai (Redis Stack) | langchain4j (PGVector) |
| :--- | :---: | :---: |
| 문서 리더 (Docx·Hwp·Hwpx·Markdown·Pdf) | ✅ | ✅ |
| 문서 업로드 (현재 `.md`) | ✅ | ✅ |
| 하이브리드 검색 (벡터 + 키워드) | ✅ | ✅ |
| 프롬프트 기법 데모 (zero-shot ~ quality-check) | ✅ | ✅ |
| 스트리밍 응답 (RAG·simple) | ✅ | ✅ |
| 세션 기반 대화 이력 | ✅ (Redis) | ✅ (JPA/PostgreSQL) |
| 구조화 출력(JSON) 데모 | ✅ | — |
| 검색 품질 평가(recall) 테스트 | — | ✅ |

- 저장소·설정 방식의 차이는 각 기술 스택의 특성에 따른 의도된 차이이다.
- `—` 항목은 아직 한쪽에만 있는 기능으로, 다른 모듈로의 교차 구현 기여를 환영한다.
- 새 기능을 추가할 때는 가급적 두 모듈에 함께 반영하여 패리티를 유지한다.
...
| 항목 | Windows | macOS |
| :---| :---| :---|
| CPU | AMD Ryzen AI 9 | Apple M4 |
| 메모리 | 32GB | 24GB |
| GPU | Nvidia RTX 4060 Laptop GPU | Apple 통합 GPU |
| GPU 메모리(VRAM) | 8GB | 통합 메모리 공유 방식 |
...
| LLM | openai/gpt-oss-20b | 20B |
| LLM | naver-hyperclovax/HyperCLOVAX-SEED-Text-Instruct-1.5B | 1.5B |
| LLM | naver-hyperclovax/HyperCLOVAX-SEED-Vision-Instruct-3B | 3B |
| LLM | LGAI-EXAONE/EXAONE-3.5-7.8B-Instruct | 7.8B |
| LLM | kakaocorp/kanana-1.5-8b-base | 8B |
