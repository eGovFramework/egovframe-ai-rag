...
- 현재 인덱싱 가능한 문서의 종류는 마크다운 파일, PDF 파일, Excel(xlsx) 파일, HWP(한글) 파일이다.
- `application.yml`의 문서 경로 관련 속성(`document.path`, `document.pdf-path`, `document.xlsx-path`, `document.hwp-path`)에서 경로를 지정한다.
- Excel 인덱싱은 경로 기반으로만 지원된다. `document.xlsx-path` 속성이 설정되지 않은 경우 Excel 처리는 건너뛰며 앱 기동에는 영향이 없다.
  - 활성화 예시: `document.xlsx-path: file:C:/workspace-test/upload/data/**/*.xlsx`
...
- 파일 업로드(웹 UI)는 마크다운(.md) 파일만 지원한다. PDF, Excel(xlsx) 및 HWP는 서버 측 경로에 파일을 직접 배치한 뒤 재인덱싱으로 처리한다.

## 실행

1. 애플리케이션을 실행하면 도큐먼트 생성 및 임베딩, 적재가 실행된다. 수동으로 실행하려면 메인 화면의 `문서 재인덱싱` 버튼을 클릭한다.
2. `문서 업로드` 버튼으로 Markdown(.md) 파일을 업로드할 수 있다. PDF, Excel(xlsx) 및 HWP 파일은 서버 경로에 직접 배치 후 재인덱싱으로 처리한다.
