...
- 현재 인덱싱 가능한 문서의 종류는 마크다운(`.md`), PDF(`.pdf`), DOCX(`.docx`), Excel(`.xlsx`), HWP(`.hwp`), HWPX(`.hwpx`) 파일이다.
- `application.yml`의 문서 경로 관련 속성(`document.path`, `document.pdf-path`, `document.docx-path`, `document.xlsx-path`, `document.hwp-path`, `document.hwpx-path`)에서 경로를 지정한다.
- DOCX/Excel/HWP/HWPX 인덱싱은 경로 기반으로만 지원된다. 각 경로 속성이 설정되지 않은 경우 해당 처리는 건너뛰며 앱 기동에는 영향이 없다.
  - DOCX 활성화 예시: `document.docx-path: file:C:/workspace-test/upload/data/**/*.docx`
  - Excel 활성화 예시: `document.xlsx-path: file:C:/workspace-test/upload/data/**/*.xlsx`
...
- 파일 업로드(웹 UI)는 마크다운(.md) 파일만 지원한다. PDF, DOCX, Excel(xlsx), HWP, HWPX는 서버 측 경로에 파일을 직접 배치한 뒤 재인덱싱으로 처리한다.
...
2. `문서 업로드` 버튼으로 Markdown(.md) 파일을 업로드할 수 있다. PDF, DOCX, Excel(xlsx), HWP, HWPX 파일은 `application.yml`의 각 경로 속성(`document.pdf-path`, `document.docx-path`, `document.xlsx-path`, `document.hwp-path`, `document.hwpx-path`)에 지정한 서버 경로에 직접 배치한 뒤 재인덱싱한다.
...
│   │       │   ├── EgovExcelReader.java      # Excel(xlsx) 문서 리더
