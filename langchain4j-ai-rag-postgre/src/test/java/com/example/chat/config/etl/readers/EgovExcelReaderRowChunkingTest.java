...
class EgovExcelReaderRowChunkingTest {
    ...
    private List<Document> parse(EgovExcelReader reader, Resource resource) throws Exception {
        Method method = EgovExcelReader.class.getDeclaredMethod("parseExcelDocument", Resource.class);
    ...
    void largeSheetSplitsIntoMultipleDocuments() throws Exception {
        ...
        assertThat(documents.get(0).metadata().getString("chunk_index")).isEqualTo("1");
        assertThat(documents.get(0).metadata().getString("chunk_count")).isEqualTo("3");
        assertThat(documents.get(0).text()).contains("row-0").contains("row-1").contains("row-2")
                .doesNotContain("row-3");
        assertThat(documents.get(2).text()).contains("row-6").doesNotContain("row-5");
    }
    ...
    void smallSheetProducesSingleDocument() throws Exception {
        ...
        assertThat(documents.get(0).metadata().getString("chunk_index")).isEqualTo("1");
        assertThat(documents.get(0).metadata().getString("chunk_count")).isEqualTo("1");
        for (int i = 0; i < 5; i++) {
            assertThat(documents.get(0).text()).contains("row-" + i);
        }
    }
    ...
    void chunkIdsAreUnique() throws Exception {
        ...
        List<String> ids = documents.stream().map(d -> d.metadata().getString("id")).toList();
        assertThat(ids).doesNotHaveDuplicates();
        assertThat(ids).allMatch(id -> id.matches("excel-sample_sheet1_chunk\\d+"));
    }
}
